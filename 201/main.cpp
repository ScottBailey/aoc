...
void dump(const tile::map_t& m)
{
   for(const auto& [n,t] : m)
      t.dump();
}


tile::map_t load(std::istream& is=std::cin)
{
   tile::map_t rv;
   while(!is.eof())
   {
...
int main(int,char**)
{
   ...
   tile::map_t tile_map = load();
   //dump(m);

   std::map<tile::bits_t,std::vector<unsigned>> counts;
   for(const auto& [n,t] : tile_map)
   {
   ...
   if(corner_count != 4)
   {
      std::cerr << "corner_count is bad: " << corner_count << "\n";
      return -1;
   }
